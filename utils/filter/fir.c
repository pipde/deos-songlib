...
main(int argc,char **argv)
    ...
    if (argc-argIndex == 0)
        ...
        {
        fprintf(stderr,"usage: fir -h -a<N.M> -w<N.M> -d<N.M> -l<N.M> "
    ...
    writeRRAHeader(out,h,"modifiedBy: fir",0);
    ...
    for (i = 0; i < h->channels; ++i)
        ...
        last[i][0] =
            readRRAAmplitude(in,out,h->bitsPerSample,outputComment) / max;
...
readCoefficients(char *fileName)
    ...
    while (fscanf(fp," %lf",&coeff) != -1)
        ...
        if (numberOfTaps >= MAX_TAPS)
            Fatal("fir: too many coefficients!\n");
