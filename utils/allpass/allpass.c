...
char *PROGRAM_NAME = "allpass";
char *PROGRAM_VERSION = "0.01";

#define getFiltered(x,y,s) (y < 0 ? 0 : rinterp(filtered[x],s,y))
#define getUnfiltered(x,y,s) (y < 0 ? 0 : rinterp(unfiltered[x],s,y))

static double allpass(int,int,int);
...
main(int argc,char **argv)
    ...
    if (argc-argIndex == 0)
        ...
        {
        fprintf(stderr,"usage: allpass -h -a<N.M> -w<N.M> -d<N.M> -l<N.M> "
    ...
    writeRRAHeader(out,h,"modifiedBy: allpass",0);
    ...
    for (currentIndex = 0; currentIndex < h->samples; ++currentIndex)
        for (j = 0; j < h->channels; ++j)
            {
            double amp = readRRAAmplitude(in,h->bitsPerSample,0) / max;

            unfiltered[j][currentIndex] = amp;
            filtered[j][currentIndex] = allpass(j,currentIndex,h->samples);
...
static double
allpass(int channel,int index,int samples)
    {
    int delayed = index - delay;

    return gain * getUnfiltered(channel,index,samples) +
              (1 - gain * gain) * 
                  (getUnfiltered(channel,delayed,samples) + gain *
                  getFiltered(channel,delayed,samples));
    }
